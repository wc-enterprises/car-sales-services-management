...
import { IEstimate, IService } from "../utils/estimates.types";
...
export class EstimateFormComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.estimateService.totalEstimatesCreatedSoFar().then((count) => {
            this.currentEstimateNumber = this.frameEstimateNumber(count);
            this.form.get("estimateNumber")?.setValue(this.currentEstimateNumber);
        });
    ...
    frameEstimateNumber(numberOfExistingEstimates: number) {
        const nextEstimateNumber = `${numberOfExistingEstimates + 1}`;
        switch (nextEstimateNumber.length) {
            case 1:
                return "#0000" + nextEstimateNumber;
            case 2:
                return "#000" + nextEstimateNumber;
            case 3:
                return "#00" + nextEstimateNumber;
            case 4:
                return "#0" + nextEstimateNumber;
            default:
    ...
    async ngOnInit(): Promise<void> {
        ...
        /** Check for estimate draft */
        const estimateDraft = localStorage.getItem("estimateDraft");
        if (estimateDraft) {
            const parsedEstimate: Partial<IEstimate> = JSON.parse(estimateDraft);
            const services = parsedEstimate.services;

            this.form.patchValue({ ...parsedEstimate, date: new Date() });
        ...
        this._inventoryService
            ...
            .catch((err) => {
                console.error(
                    "Errored while fetching services in add estimate component"
                );
                this.allServices = [];
            });
    }
    ...
    clearForm(resetToType?: "SERVICE" | "SALE") {
        ...
        this.form.patchValue({
            estimateNumber: this.currentEstimateNumber,
    ...
    showCarDetails() {
        ...
        // If estimate type is services, show car details in the add form.
    ...
    async numberInformation() {
        this.numberList = await this.estimateService.getNumberOfContacts();
    }
    async getContactList() {
        this.contactList = await this.estimateService.getNameOfContacts();
    }
    async getRegno() {
        this.regNoList = await this.estimateService.getRegNo();
    }
    async getMakeName() {
        this.makeNameList = await this.estimateService.makeName();
    }
    async getMakeForMaping() {
        this.mapForMake = await this.estimateService.makeMap();
    }
    // CustomerNames
    filterNames() {
    ...
    selectName(name: string) {
        ...
        this.estimateService.mapName(name).then((data) => {
            if (!data) return;
    ...
    selectRegNo(regNoList: string) {
        ...
        this.estimateService.mapRegNo(regNoList).then((data) => {
            if (!data) return;
    ...
    async onSave() {
        if (this.form.valid) {
            ...
            if (
                ...
                const billToCustomer = formData.billTo as IEstimate["billTo"];
            ...
            const estimateId = await this.estimateService.createEstimate(formData);

            localStorage.removeItem("estimateDraft");
            this.router.navigate([
                `/inventory-and-invoice/estimate-billing/preview/${estimateId}`,
            ]);
            this.estimateService.incrementTotalEstimatesCreated();
        } else {
