...
export class EstimatesListComponent implements OnInit, AfterViewInit, OnDestroy {
    @ViewChild(MatSort) private _sort: MatSort;

    estimates$: Observable<IEstimate[]>;
    estimates: IEstimate[];
    filteredCustomers: Contact[];
    flashMessage: "success" | "error" | null = null;
    isLoading: boolean = false;
    searchInputControl: UntypedFormControl = new UntypedFormControl();
    selectedEstimate: IEstimate | null = null;
    selectedProductForm: UntypedFormGroup;
    tagsEditMode: boolean = false;
    searchQuery: string;
    private _unsubscribeAll: Subject<any> = new Subject<any>();

    selectedTimePeriodFilter: TEstimateTimeFilter = "";
    selectedEstimateTypeFilter: TEstimateTypeFilter = "";
    dateRange = {
        startDate: Date.now(),
        endDate: Date.now(),
    };
    ...
    getDisplayValueOfSelectedFilter(
        actualValue: TEstimateTimeFilter | TEstimateTypeFilter,
        filterType: "TIME_PERIOD" | "ESTIMATE_TYPE"
    ) {
        if (filterType === "ESTIMATE_TYPE") {
            if (actualValue === "ALL") return "All";
            if (actualValue === "SALE") return "Sales";
            if (actualValue === "SERVICE") return "Services";
            if (actualValue === "") return "Filter by Estimate Type";
        }
    ...
    setFilterValue(
        value: TEstimateTimeFilter | TEstimateTypeFilter,
        type: "TIME_PERIOD" | "ESTIMATE_TYPE"
    ) {
        if (type === "ESTIMATE_TYPE")
            this.selectedEstimateTypeFilter = value as TEstimateTypeFilter;
        if (type === "TIME_PERIOD")
            this.selectedTimePeriodFilter = value as TEstimateTimeFilter;

        if (value === "dr") {
            this.openDateRangeDialog();
        } else {
            this._estimatesService.getEstimates(
                this.selectedTimePeriodFilter,
                this.selectedEstimateTypeFilter,
                {
    ...
    constructor(
        ...
        private _estimatesService: EstimateBillingService,
    ...
    navigateToForm() {
        this.router.navigate(["/inventory-and-invoice/estimate-billing/add"]);
    }
    ...
    ngOnInit(): void {
        // Get the products
        this.estimates$ = this._estimatesService.estimates$;
        this._estimatesService.estimates$.subscribe((data) => {
            this.estimates = data;
        });
    ...
    previewEstimate(id: string) {
        ...
        this.router.navigate([`/inventory-and-invoice/estimate-billing/preview/${id}`]);
    }
    ...
    toggleDetails(carId: string): void {
        ...
        if (this.selectedEstimate && this.selectedEstimate.id === carId) {
            // Close the details
            this.closeDetails();
            return;
        }
        ...
        this._estimatesService.getEstimateById(carId).subscribe((product) => {
            // Set the selected product
            this.selectedEstimate = product;
    ...
    updateSelectedProduct(): void {
        ...
        this._estimatesService.updateEstimate(product.id, product).then(() => {
    ...
    deleteEstimate(id: string): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === "confirmed") {
                ...
                this._estimatesService.deleteEstimate(id).then(() => {
                    // Close the details
                    this.closeDetails();
                });
            }
    ...
    async generateEstimatesReport() {
        // Load the existing PDF template
        // NOTE: Using the same template as Invoices for now, might need separate one if provided
        ...
        this.estimates.forEach((estimate) => {
            console.log(
                ...
                "Tax type",
                typeof estimate.tax?.value
            );
            if (estimate.tax && estimate.tax.value) totalTax += +estimate.tax?.value;
        ...
        let tableContents: string[][] = [
            ...this.estimates.map((estimate, index) => [
                DateTime.fromISO(new Date(estimate.date).toISOString()).toFormat(
                    "dd/MM/yy"
                ),
                estimate.estimateNumber,
                estimate.services.map((item) => item.item).join(", "),
                estimate.type,
                estimate.billTo.name,
                estimate.carInfo?.make ?? "-",
                estimate.tax?.value ? estimate.tax.value.toString() : "0",
        ...
        while (tableContents.length > 0) {
            ...
            // Inner loop creates the table by adding one estimate after the other until it throw the error : "Table height exceeds the available space on the page."
            const estimatesForPage = [];
            for (let i = 0; i < tableContents.length; i++) {
                estimatesForPage.push(tableContents[i]);
                try {
                    /**
                    ...
                    if (
                        ...
                        tableContents = tableContents.slice(estimatesForPage.length);
                        break;
                    }
                }
            }
        ...
        link.download = `Estimate_Report_${formattedDate}_${formattedTime}.pdf`;
        link.click();
    }
}
